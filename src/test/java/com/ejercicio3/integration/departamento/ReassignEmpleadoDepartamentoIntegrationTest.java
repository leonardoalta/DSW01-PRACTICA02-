...
class ReassignEmpleadoDepartamentoIntegrationTest extends BaseIntegrationTest {

    @Test
    void shouldReassignEmpleadoBetweenDepartamentos() throws Exception {
        long depA = createDepartamento("Logistica", "Area logistica");
        long depB = createDepartamento("Comercial", "Area comercial");
        String claveEmpleado = createEmpleado("Diego", "5554000", "Password123");

        mockMvc.perform(put("/api/v1/departamentos/{departamentoId}/empleados/{claveEmpleado}", depA, claveEmpleado)
                        .with(httpBasic(AUTH_USER, AUTH_PASSWORD)))
                .andExpect(status().isOk());

        mockMvc.perform(put("/api/v1/departamentos/{departamentoId}/empleados/{claveEmpleado}", depB, claveEmpleado)
                        .with(httpBasic(AUTH_USER, AUTH_PASSWORD)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.departamentoId").value(depB));

        mockMvc.perform(get("/api/v1/departamentos/{id}", depA)
                        .with(httpBasic(AUTH_USER, AUTH_PASSWORD)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.cantidadEmpleados").value(0));

        mockMvc.perform(get("/api/v1/departamentos/{id}", depB)
                        .with(httpBasic(AUTH_USER, AUTH_PASSWORD)))
                .andExpect(status().isOk())
